...
let rec codegen_expr (env : env) (func : Llvm.llvalue) = function
  | ExpConst (ConstInt n) -> Llvm.const_int i64_t n
  | ExpConst (ConstBool b) -> Llvm.const_int i64_t (if b then 1 else 0)
  ...
  | ExpIfElse (cond_expr, then_expr, else_expr) ->
    ...
    Llvm.build_phi [ then_val, then_exit; else_val, else_exit ] "ifresult" builder
  | ExpLetIn (Notrec, name, e1, e2) ->
    let v = codegen_expr env func e1 in
    env_set env name v;
    codegen_expr env func e2
  | ExpApp (ExpVar (fname, _), arg, _) ->
    let callee = lookup_func fname in
    let arg_val = codegen_expr env func arg in
    let ft = Llvm.function_type i64_t [| i64_t |] in
    Llvm.build_call ft callee [| arg_val |] "call" builder
  | e -> failwith (Printf.sprintf "Unsupported expression: %s" (show_expression e))
...
let codegen_main expr =
  ...
  let print_int_fn = lookup_func "print_int" in
  let print_int_ft = Llvm.function_type void_t [| i64_t |] in
  let _ = Llvm.build_call print_int_ft print_int_fn [| result |] "" builder in
  let _ = Llvm.build_ret (Llvm.const_int i32_t 0) builder in
  fn
;;
...
let codegen_binding = function
  ...
  | Let (Notrec, [ (PatVar (name, _), body) ]) ->
    ...
    ()
  | Exp _ -> ()
  | b -> failwith (Printf.sprintf "Unsupported binding: %s" (show_bindings b))
;;

let codegen_program stmts output_file =
  let _ =
    Llvm.declare_function "print_int" (Llvm.function_type void_t [| i64_t |]) the_module
  in
  List.iter codegen_binding stmts;
