...
let lookup_func md name =
  match Llvm.lookup_function name md with
  | Some f -> Ok f
  | None -> Error (Printf.sprintf "Unknown function: %s" name)
;;

let build_make_closure md fn_ptr arity =
  let* mc_fn = lookup_func md "make_closure" in
  Ok
    (Llvm.build_call
       make_closure_ft
       mc_fn
...
let build_apply md fv av =
  let* apply_fn = lookup_func md "apply" in
...
let build_closure_of md fname arity =
  let* f = lookup_func md fname in
  let fn_ptr = Llvm.build_ptrtoint f i64_t "fptr" builder in
  build_make_closure md fn_ptr arity
;;
...
let apply_chain md start args =
  List.fold_left
    (fun acc av ->
       let* fv = acc in
       build_apply md fv av)
    start
    args
;;

let rec codegen_cexpr md arities (env : env) (func : Llvm.llvalue) = function
  | CImm i -> codegen_imm md arities env i
  | CBinop (op, le, re) ->
    let* l = codegen_imm md arities env le in
    let* r = codegen_imm md arities env re in
    let bool_op icmp_op =
      Llvm.build_select
  ...
  | CApp (f, args) ->
    ...
    then apply_chain md (load_global md f) argvals (* top-level value, applied *)
      ...
      then
        ...
        let* callee = lookup_func md f in
        let ft = Llvm.function_type i64_t (Array.make arity i64_t) in
        Ok (Llvm.build_call ft callee (Array.of_list argvals) "call" builder)
      else apply_chain md (build_closure_of md f arity) argvals)
  | CTuple fields ->
    let* vals = map_result (codegen_imm md arities env) fields in
    ...
    List.iteri
      (fun i v ->
         ...
         ())
      vals;
    let* ct = lookup_func md "create_tuple" in
    Ok (Llvm.build_call create_tuple_ft ct [| i64v n; arr |] "tuple" builder)
  | CField (v, i) ->
    let* tv = codegen_imm md arities env v in
    let* fld = lookup_func md "field" in
    Ok (Llvm.build_call field_ft fld [| tv; tag_int i |] "field" builder)
  | CIf (cond, then_a, else_a) ->
    let* cond_val = codegen_imm md arities env cond in
    ...
    let* then_val = codegen_aexpr md arities env func then_a in
    let then_exit = Llvm.insertion_block builder in
    let _ = Llvm.build_br merge_bb builder in
    Llvm.position_at_end else_bb builder;
    let* else_val = codegen_aexpr md arities env func else_a in
...
and codegen_aexpr md arities (env : env) (func : Llvm.llvalue) = function
  | ACExpr c -> codegen_cexpr md arities env func c
  | ALet (name, c, rest) ->
    let* v = codegen_cexpr md arities env func c in
    env_set env name v;
    codegen_aexpr md arities env func rest
;;
...
let codegen_func md arities (f : afunc) =
  let n = List.length f.params in
  let ft = Llvm.function_type i64_t (Array.make n i64_t) in
  let fn =
    match Llvm.lookup_function f.name md with
    | Some x -> x
    | None -> Llvm.declare_function f.name ft md
  in
...
let codegen_cfunc md arities cafs (f : afunc) =
  ...
  then (
    ...
    match Llvm.lookup_function f.name md with
    | None ->
      let _ = Llvm.declare_function f.name ft md in
      let* _ = codegen_func md arities f in
      Ok ()
    | Some _ ->
      let* _ = codegen_func md arities f in
      Ok ())
...
let codegen_program (prog : aprogram) output_file =
  ...
  List.iter
    (fun name ->
       ...
       ())
    builtins;
  let _ = Llvm.declare_function "make_closure" make_closure_ft md in
  let _ = Llvm.declare_function "apply" apply_ft md in
  ...
  let* () =
    List.fold_left
      ...
      prog
  in
  Ok (Llvm.print_module output_file md)
;;
